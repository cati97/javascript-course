...
for (const [key, value] of oddEntries) {
  ...
  console.log(`Odd of ${teamName ? `victory ${teamName}` : "draw"}: ${value}`);
}
...
for (const scorePlayer of game.scored) {
  scorers[scorePlayer] = (scorers[scorePlayer] || 0) + 1;
  // create new key value pair if the key doesn't exist
  // if it exists already - add one to it
}
